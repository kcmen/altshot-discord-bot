...
# /view_score command
@app_commands.command(name="view_score", description="View the score submitted for a specific team")
@app_commands.describe(team="Team to look up (e.g. T4 or just 4)")
async def view_score(interaction: discord.Interaction, team: str):
    team = team.strip().upper()
    if not team.startswith("T"):
        team = f"T{team}"
    conn = sqlite3.connect("scores.db")
    cursor = conn.cursor()
    cursor.execute("SELECT result, week, submitted_by, timestamp FROM scores WHERE team = ? ORDER BY id DESC LIMIT 1", (team,))
    row = cursor.fetchone()
    conn.close()
    if row:
        result, week, submitted_by, timestamp = row
        await interaction.response.send_message(f"🔍 **{team}** submitted `{result}` (Week {week}) by **{submitted_by}** on `{timestamp}`")
    else:
